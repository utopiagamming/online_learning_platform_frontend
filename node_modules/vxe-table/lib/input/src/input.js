...
function toFloatValueFixed(inputValue, digitsValue) {
  if (/^-/.test('' + inputValue)) {
    return _ctor.default.toFixed(_ctor.default.ceil(inputValue, digitsValue), digitsValue);
  }

  return _ctor.default.toFixed(_ctor.default.floor(inputValue, digitsValue), digitsValue);
}
  ...
  computed: {
    ...
    weekDatas: function weekDatas() {
      ...
      if (this.isDatePicker) {
        ...
        var sWeek = _ctor.default.toNumber(_ctor.default.isNumber(startDay) || _ctor.default.isString(startDay) ? startDay : _ctor.default.isNumber(startWeek) ? startWeek : this.dateOpts.startWeek);
  ...
  methods: {
      ...
      if (!inpAttrs.readonly) {
        if (isNumber) {
          if (value) {
            ...
            this.emitUpdate(type === 'float' ? toFloatValueFixed(inpVal, digitsValue) : _ctor.default.toString(inpVal), {
              type: 'check'
            });
          }
        } else if (isDatePicker) {
    ...
    numberChange: function numberChange(isPlus, evnt) {
      ...
      if (this.vaildMinNum(newValue) && this.vaildMaxNum(newValue)) {
        this.emitUpdate(type === 'float' ? toFloatValueFixed(newValue, digitsValue) : _ctor.default.toString(newValue), evnt);
      }
    },
    // 数值
    // 日期
