...
function getOrderField(_vm, column) {
  ...
  if (sortBy && _ctor.default.isArray(sortBy)) {
    return sortBy;
  }
  ...
  return function (row) {
    ...
    if (sortBy) {
      cellValue = _ctor.default.isFunction(sortBy) ? sortBy({
        row: row,
        column: column
      }) : _ctor.default.get(row, sortBy);
    } else {
      cellValue = _vm.getCellLabel(row, column);
    }

    if (!sortType || sortType === 'auto') {
      return isNaN(cellValue) ? cellValue : _ctor.default.toNumber(cellValue);
    } else if (sortType === 'number') {
      return _ctor.default.toNumber(cellValue);
    } else if (sortType === 'string') {
      return _ctor.default.toValueString(cellValue);
    }
...
var Methods = {
  ...
  updateCache: function updateCache(source) {
    ...
    var handleCache = function handleCache(row, index, items, path, parent) {
      ...
      if (_tools.UtilTools.eqEmptyValue(rowid)) {
    ...
    return this.createData(childRecords).then(function (rows) {
      ...
      if (keepSource) {
        ...
        var matchObj = _ctor.default.findTree(tableSourceData, function (item) {
          return rowid === getRowid(_this7, item);
        }, treeOpts);
      ...
      _ctor.default.eachTree(rows, function (childRow, index, items, path, parent) {
        var rowid = getRowid(_this7, childRow);
        var rest = {
          row: childRow,
          rowid: rowid,
          index: -1,
  ...
  defineField: function defineField(record) {
    ...
    ohterFields.forEach(function (key) {
      if (key && _tools.UtilTools.eqEmptyValue(_ctor.default.get(record, key))) {
    ...
    if (_tools.UtilTools.eqEmptyValue(_ctor.default.get(record, rowkey))) {
  ...
  revertData: function revertData(rows, field) {
    ...
    if (rows) {
      ...
      targetRows = _ctor.default.toArray(this.getUpdateRecords());
    }
  ...
  updateAfterFullData: function updateAfterFullData() {
    ...
    var column = _ctor.default.find(tableFullColumn, function (column) {
      return column.order;
    });
      ...
      tableData = tableData.filter(function (row) {
        return filterColumns.every(function (_ref2) {
          ...
          if (valueList.length && !(filterOpts.remote || remoteFilter)) {
            ...
            return valueList.indexOf(_ctor.default.get(row, property)) > -1;
          }
    ...
    if (column && column.order) {
      ...
      if (!isRemote) {
        if (allSortMethod) {
          ...
          var rest = column.sortMethod ? tableData.sort(column.sortMethod) : _ctor.default.orderBy(tableData, getOrderField(this, column));
        ...
        if (operCtxMenu) {
          ...
          if (!isEditStatus) {
            ...
            if (keyboardOpts.isDel && (selected.row || selected.column)) {
              ...
              // 如果树形表格回退键关闭当前行返回父节点
              var _XEUtils$findTree = _ctor.default.findTree(_this21.afterFullData, function (item) {
      ...
      if (reserve) {
        if (value) {
          selectRows.forEach(function (row) {
            checkboxReserveRowMap[getRowid(_this26, row)] = row;
          });
        } else {
  ...
  handleDefaultSort: function handleDefaultSort() {
    ...
    if (defaultSort) {
      ...
      if (field && order) {
        var column = _ctor.default.find(this.tableFullColumn, function (item) {
  ...
  getSortColumn: function getSortColumn() {
    ...
    return _ctor.default.find(this.tableFullColumn, function (column) {
  ...
  handleAsyncRowExpand: function handleAsyncRowExpand(row) {
    ...
    return new Promise(function (resolve) {
      ...
      _this31.expandOpts.loadMethod({
        ...
        _ctor.default.remove(_this31.expandLazyLoadeds, function (item) {
          return item === row;
        });
  ...
  scrollToTreeRow: function scrollToTreeRow(row) {
    ...
    return Promise.all(rests).then(function () {
      return _tools.DomTools.rowToVisible(_this40, row);
    });
  },
